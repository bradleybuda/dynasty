class Game
  ...
  MAXIMUM_SEASONS_REQUIRED = ((CHAMPIONSHIPS_TO_WIN - 1) * TEAM_COUNT) + 1
  ...
  raise unless PERMUTATIONS.size == MAXIMUM_SEASONS_REQUIRED
  ...
  attr_reader :victor, :season, :picks_made

  def initialize(teams)
    raise unless teams.size == TEAM_COUNT
    @teams = teams
    @played = false
    @picks_made = Array.new(MAXIMUM_SEASONS_REQUIRED) { Array.new(PICKS_PER_ROUND) }
    @season = -1
  end
  ...
  def play!
    ...
    draft_order = (0...MAXIMUM_SEASONS_REQUIRED).map do |season|
    ...
    while @teams.all? { |t| t.championships < 3 }
      ...
      (0...PICKS_PER_ROUND).each do |pick_index|
        ...
        @picks_made[@season][pick_index] = next_free_agent
